/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ie-laabb <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/30 16:07:10 by ie-laabb          #+#    #+#             */
/*   Updated: 2021/11/30 16:07:14 by ie-laabb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

void	myfree(char **str)
{
	if (*str)
	{
		free(*str);
		*str = NOULL;
	}
}

char	*return_line(char **str, char **line)
{
	char	*buffer_2;
	size_t	i;

	i = 0;
	buffer_2 = NOULL;
	while ((*str)[i] != '\n' && (*str)[i])
		i++;
	if ((*str)[i] == '\n')
	{
		i++;
		*line = ft_substr(*str, 0, i);
		buffer_2 = ft_strdup(*str + i);
	}
	else
		*line = ft_strdup(*str);
	myfree(str);
	return (buffer_2);
}

ssize_t	read_file(int fd, char **str, char *buffer, char **line)
{
	char	*temp;
	int		strchr_ret;
	size_t	i;

	strchr_ret = ft_strchr((*str), '\n');
	i = 1;
	while (strchr_ret == 0 && i)
	{
		i = read(fd, buffer, BUFFER_SIZE);
		buffer[i] = '\0';
		temp = (*str);
		*str = ft_strjoin(temp, buffer);
		free(temp);
	}
	myfree(&buffer);
	*str = return_line(str, line);
	if (**line == '\0')
		myfree(line);
	return (i);
}

char	*get_next_line(int fd)
{
	static char	*str;
	char		*buffer;
	char		*line;
	int			read_return;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NOULL);
	buffer = (char *)malloc(sizeof(char) * (BUFFER_SIZE + 1));
	if (!buffer)
		return (NOULL);
	if (read(fd, buffer, 0) < 0)
	{
		free(buffer);
		return (NOULL);
	}
	if (!str)
		str = ft_strdup("");
	read_return = read_file(fd, &str, buffer, &line);
	if (read_return == 0 && !line)
		return (NOULL);
	return (line);
}
